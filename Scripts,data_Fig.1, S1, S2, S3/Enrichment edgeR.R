...
wspF_c <- read.csv("output/edgeR_norm/Stat Tests/CwspF.csv")
...
wrinkly_cf <- read.csv("output/edgeR_norm/Stat Tests/Cwrinkly_colony_sig_fdr.csv")
rownames(wrinkly_cf) <- wrinkly_cf$X
wrinkly_cf$X <- NULL

wrinkly_lf <- read.csv("output/edgeR_norm/Stat Tests/Lwrinkly_liquid_sig_fdr.csv")
...
for(i in 1:length(enrich.results)){
  write.csv(enrich.results[[i]], file = paste("output/edgeR_norm/GSEA/GO_", names(enrich.results)[i], ".csv", sep = ""))
}

#plot GSEA test

GSEA_wcf <- read.csv("output/edgeR_norm/GSEA/GO_wrinkly_cf.csv")
GSEA_wlf <- read.csv("output/edgeR_norm/GSEA/GO_wrinkly_lf.csv")
